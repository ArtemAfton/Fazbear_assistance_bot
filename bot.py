...
def start(message: telebot.types.Message):
    create_database()
    bot.send_message(message.from_user.id, "Интерком приветствует тебя! Отправьте мне текстовое или голосвое сообщение и я "
                                           "отвечу тебе!")
...
def stt_handler(message: telebot.types.Message):
    user_id = message.from_user.id
    bot.send_message(user_id, 'Отправь мне голосовое сообщение, чтобы я его распознал!')
...
def stt(message: telebot.types.Message):
    ...
    if not message.voice:
        bot.send_message(user_id, 'Получено не голосовое сообщение')
        bot.register_next_step_handler(message, stt)
        return
